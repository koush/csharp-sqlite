...
#else
static void TRACE( string X, params object[] ap ) { }//# define TRACE(X)
#endif
...
*/
static int sqlite3_enable_shared_cache( bool enable )
{
...
*/
    ...
    static int querySharedCacheTableLock( Btree a, Pgno b, u8 c ) { return SQLITE_OK; }
    //#define setSharedCacheTableLock(a,b,c) SQLITE_OK
    static int setSharedCacheTableLock( Btree a, Pgno b, u8 c ) { return SQLITE_OK; }
...
*/
if ( pBt.pWriter != p && pBt.isExclusive != 0 )
{
...
*/
if (
( p.db.flags & SQLITE_ReadUncommitted ) != 0 &&
( eLock == READ_LOCK ) &&
iTable != MASTER_ROOT
)
{
...
if ( null == pLock )
{
pLock = new BtLock();// (BtLock*)sqlite3MallocZero( sizeof( BtLock ) );
if ( null == pLock )
{
return SQLITE_NOMEM;
}
pLock.iTable = (int)iTable;
pLock.pBtree = p;
...
Debug.Assert( sqlite3BtreeHoldsMutex( p ) );
Debug.Assert( p.sharable || null == ppIter );
Debug.Assert( p.inTrans > 0 );

while ( ppIter != null )
{
BtLock pLock = ppIter;
Debug.Assert( pBt.isExclusive == 0 || pBt.pWriter == pLock.pBtree );
Debug.Assert( pLock.pBtree.inTrans >= pLock.eLock );
if ( pLock.pBtree == p )
{
if ( ppIter == pBt.pLock ) pBt.pLock = pLock.pNext;
else pLock.pNext = pLock.pNext.pNext;
ppIter = pLock.pNext;
...
Debug.Assert( pBt.isPending == 0 || pBt.pWriter != null );
if ( pBt.pWriter == p )
{
pBt.pWriter = null;
pBt.isExclusive = 0;
...
*/
    static int btreeSetHasContent( BtShared pBt, u32 pgno )
    {
      int rc = SQLITE_OK;
    ...
    */
    static bool btreeGetHasContent( BtShared pBt, u32 pgno )
    {
    ...
    */
    static int saveCursorPosition( BtCursor pCur )
    {
      ...
      ** data.
      */
      if ( rc == SQLITE_OK && pCur.apPage[0].intKey == 0 )
      {
        byte[] pKey = new byte[pCur.nKey];// sqlite3Malloc( (int)pCur.nKey );
        //if (pKey)
        //{
        rc = sqlite3BtreeKey( pCur, 0, (int)pCur.nKey, ref pKey );
        if ( rc == SQLITE_OK )
        {
          pCur.pKey = pKey;
        }
    ...
    */
    static void sqlite3BtreeClearCursor( BtCursor pCur )
    {
      ...
      Debug.Assert( cursorHoldsMutex( pCur ) );
#endif
      pCur.pKey = null;//sqlite3_free( ref pCur.pKey );
    ...
    ** saveCursorPosition().
    */
    static int sqlite3BtreeRestoreCursorPosition( BtCursor pCur )
    {
      int rc;
#if !NDEBUG
      Debug.Assert( cursorHoldsMutex( pCur ) );
      ...
      pCur.eState = CURSOR_INVALID;
      rc = sqlite3BtreeMoveto( pCur, pCur.pKey, pCur.nKey, false, ref pCur.skip );
      rc = 0;
      if ( rc == SQLITE_OK )
      {
        pCur.pKey = null;//sqlite3_free( ref pCur.pKey );
    ...
    static int restoreCursorPosition( BtCursor pCur )
    {
      if ( pCur.eState == CURSOR_REQUIRESEEK )
        return sqlite3BtreeRestoreCursorPosition( pCur );
      else
        return SQLITE_OK;
    }
...
** input page number.
*/
    static u32 ptrmapPageno( BtShared pBt, u32 pgno )
    {
      int nPagesPerMapPage;
      u32 iPtrMap, ret;
      Debug.Assert( sqlite3_mutex_held( pBt.mutex ) );
      nPagesPerMapPage = ( pBt.usableSize / 5 ) + 1;
      iPtrMap = (u32)( ( pgno - 2 ) / nPagesPerMapPage );
      ret = (u32)( iPtrMap * nPagesPerMapPage ) + 2;
    ...
    */
    static int ptrmapPut( BtShared pBt, Pgno key, int eType, Pgno parent )
    {
      DbPage pDbPage = new DbPage();  /* The pointer map page */
      byte[] pPtrmap;                 /* The pointer map data */
      u32 iPtrmap;                    /* The pointer map page number */
      u32 offset;                     /* Offset in pointer map page */
      int rc;
...
return SQLITE_CORRUPT_BKPT;
    ...
    ** An error code is returned if something goes wrong, otherwise SQLITE_OK.
    */
    static int ptrmapGet( BtShared pBt, u32 key, ref int pEType, ref u32 pPgno )
    {
      DbPage pDbPage = null; /* The pointer map page */
      u32 iPtrmap;           /* Pointer map page index */
      byte[] pPtrmap;        /* Pointer map page data */
      u32 offset;            /* Offset of entry in pointer map */
      int rc;
...
static int ptrmapPut( BtShared pBt, Pgno key, int eType, Pgno parent ) { return SQLITE_OK; }
//#define ptrmapGet(w,x,y,z) SQLITE_OK
static int ptrmapGet( BtShared pBt, Pgno key, int pEType, Pgno pPgno ) { return SQLITE_OK; }
#endif
...
*/
    ...
    {
      return pPage.maskPage & get2byte( pPage.aData, ( pPage ).cellOffset + 2 * ( iCell ) );
    }
    ...
    static int findOverflowCell( MemPage pPage, int iCell )
    {
      ...
      for ( i = pPage.nOverflow - 1 ; i >= 0 ; i-- )
      {
        ...
        if ( k <= iCell )
        {
          if ( k == iCell )
          {
            return -i - 1; // Negative Offset means overflow cells // return pOvfl.pCell;
          }
          iCell--;
        }
      }
    ...
    */
    ...
    { sqlite3BtreeParseCellPtr( pPage, pPage.aData, iCell, pInfo ); }
    static void sqlite3BtreeParseCellPtr(
    MemPage pPage,        /* Page containing the cell */
    byte[] pCell,         /* The actual data */
    CellInfo pInfo        /* Fill in this structure */
    )
    { sqlite3BtreeParseCellPtr( pPage, pCell, 0, pInfo ); }
    ...
    )
    {
      ...
      if ( pPage.intKey != 0 )
      {
        ...
        n += (u16)getVarint( pCell, iCell + n, ref pInfo.nKey );//    n += getVarint(pCell[n], (u64*)&pInfo.nKey);
        pInfo.nData = nPayload;
      }
      else
      {
        pInfo.nData = 0;
        n += (u16)getVarint32( pCell, iCell + n, ref nPayload );//getVarint32( &pCell[n], nPayload );
        pInfo.nKey = nPayload;
      }
    ...
    */
    ...
    static u16 cellSizePtr( MemPage pPage, byte[] pCell, int offset )
    {
      ...
      sqlite3BtreeParseCellPtr( pPage, pTemp, info );
      return info.nSize;
    }
...
** this function verifies that this invariant is not violated. */
      ...
      if ( pPage.intKey != 0 )
      {
        ...
        ** past the end of the key value. */
        ...
        while ( ( ( pCell[_pIter++] ) & 0x80 ) != 0 && _pIter < pEnd ) ;//( ( *pIter++ ) & 0x80 && pIter < pEnd ) ;
      }
      else
      {
...
*/
    static int ptrmapPutOvflPtr( MemPage pPage, byte[] pCell )
    {
      CellInfo info = new CellInfo();
    ...
    static int ptrmapPutOvflPtr( MemPage pPage, int pCell )
    {
      CellInfo info = new CellInfo();
...
*/
    static int defragmentPage( MemPage pPage )
    {
      ...
      Buffer.BlockCopy( data, cbrk, temp, cbrk, usableSize - cbrk );//memcpy(&temp[brk], data[brk], usableSize - brk);
      cbrk = usableSize;
      for ( i = 0 ; i < nCell ; i++ )
      {
...
return SQLITE_CORRUPT_BKPT;
        ...
        Debug.Assert( cbrk + size <= usableSize && cbrk >= 0 );
        Buffer.BlockCopy( temp, pc, data, cbrk, size );// memcpy(&data[brk], temp[pc], size);
        put2byte( data, pAddr, cbrk );
      }
      Debug.Assert( cbrk >= cellOffset + 2 * nCell );
      put2byte( data, hdr + 5, cbrk );
      data[hdr + 1] = 0;
      data[hdr + 2] = 0;
      data[hdr + 7] = 0;
      addr = cellOffset + 2 * nCell;
      Array.Clear( data, addr, cbrk - addr );  //memset(&data[addr], 0, brk - addr);
      Debug.Assert( sqlite3PagerIswriteable( pPage.pDbPage ) );
...
return SQLITE_CORRUPT_BKPT;
    ...
    */
    static int allocateSpace( MemPage pPage, int nByte )
    {
      ...
      */
      Debug.Assert( nByte <= (
      get2byte( data, hdr + 5 ) - ( hdr + 8 + ( pPage.leaf != 0 ? 0 : 4 ) + 2 * get2byte( data, hdr + 3 ) )
      ) );
          ...
          if ( size >= nByte )
          {
            ...
            if ( x < 4 )
            {
              ...
              ** fragmented bytes within the page. */
              data[addr + 0] = data[pc + 0]; data[addr + 1] = data[pc + 1];//memcpy(&data[addr], data[pc], 2);
    ...
    */
    static int freeSpace( MemPage pPage, int start, int size )
    {
      ...
      Debug.Assert( pPage.pBt != null );
      Debug.Assert( sqlite3PagerIswriteable( pPage.pDbPage ) );
      Debug.Assert( start >= pPage.hdrOffset + 6 + ( pPage.leaf != 0 ? 0 : 4 ) );
...
return SQLITE_CORRUPT_BKPT;
          ...
          data[pPage.hdrOffset + 7] -= (byte)frag;
      ...
      if ( data[hdr + 1] == data[hdr + 5] && data[hdr + 2] == data[hdr + 6] )
      {
        ...
        put2byte( data, hdr + 1, get2byte( data, pbegin ) ); //memcpy(&data[hdr+1], data[pbegin], 2);
    ...
    */
    static int decodeFlags( MemPage pPage, int flagByte )
    {
      ...
      Debug.Assert( sqlite3_mutex_held( pPage.pBt.mutex ) );
      pPage.leaf = (byte)( flagByte >> 3 ); Debug.Assert( PTF_LEAF == 1 << 3 );
      flagByte &= ~PTF_LEAF;
...
return SQLITE_CORRUPT_BKPT;
    ...
    */
    static int sqlite3BtreeInitPage( MemPage pPage )
    {
      Debug.Assert( pPage.pBt != null );
      Debug.Assert( sqlite3_mutex_held( pPage.pBt.mutex ) );
      ...
      if ( 0 == pPage.isInit )
      {
        ...
        hdr = pPage.hdrOffset;
        data = pPage.aData;
        if ( decodeFlags( pPage, data[hdr] ) != 0 ) //return SQLITE_CORRUPT_BKPT();
#if SQLITE_DEBUG
          return SQLITE_CORRUPT_BKPT();
#else
return SQLITE_CORRUPT_BKPT;
        ...
        pPage.cellOffset = (u16)( cellOffset = hdr + 12 - 4 * pPage.leaf );
...
return SQLITE_CORRUPT_BKPT;
        ...
        pPage.nFree = (u16)( nFree - ( cellOffset + 2 * pPage.nCell ) );
...
return SQLITE_CORRUPT_BKPT();
    ...
    */
    static void zeroPage( MemPage pPage, int flags )
    {
      ...
      Array.Clear( data, hdr + 1, 4 );//  memset(&data[hdr+1], 0, 4) ' Not sure this is needed, see above
      data[hdr + 7] = 0;
      data[hdr + 5] = (byte)( pBt.usableSize >> 8 ); data[hdr + 6] = (byte)( pBt.usableSize );// put2byte(data,hdr + 5], pBt.usableSize);
    ...
    ** the btree layer.
    */
    static MemPage btreePageFromDbPage( DbPage pDbPage, u32 pgno, BtShared pBt )
    {
    ...
    */
    ...
    u32 pgno,             /* Number of the page to fetch */
    ref MemPage ppPage,   /* Return the page in this parameter */
    u8 noContent        /* Do not load page content if true */
    )
    {
      int rc;
      DbPage pDbPage = new DbPage();

      Debug.Assert( sqlite3_mutex_held( pBt.mutex ) );
      rc = sqlite3PagerAcquire( pBt.pPager, pgno, ref pDbPage, noContent );
      if ( rc != 0 ) return rc;
    ...
    ** MemPage.aData elements if needed.
    */
    static MemPage btreePageLookup( BtShared pBt, u32 pgno )
    {
      DbPage pDbPage;
      Debug.Assert( sqlite3_mutex_held( pBt.mutex ) );
    ...
    */
    ...
    u32 pgno,             /* Number of the page to get */
...
return SQLITE_CORRUPT;
        ...
        rc = sqlite3BtreeGetPage( pBt, pgno, ref ppPage, 0 );
        if ( rc != 0 ) return rc;
        pPage = ppPage;
      }
      if ( 0 == pPage.isInit )
    ...
    */
    static void releasePage( MemPage pPage )
    {
      if ( pPage != null )
      {
        ...
        Debug.Assert( pPage.pBt != null );
        Debug.Assert( sqlite3PagerGetExtra( pPage.pDbPage ).Equals( pPage ) );
...
*/
if ( isMemdb == false && !String.IsNullOrEmpty( zFilename ) )
{
if ( sqlite3GlobalConfig.sharedCacheEnabled )
{
int nFullPathname = pVfs.mxPathname + 1;
StringBuilder zFullPathname = new StringBuilder( nFullPathname );//sqlite3Malloc( nFullPathname );
sqlite3_mutex mutexShared;
p.sharable = true;
db.flags |= SQLITE_SharedCache;
if ( null == zFullPathname )
{
//sqlite3DbFree( db, p );
return SQLITE_NOMEM;
}
...
for ( pBt = sqlite3SharedCacheList ; pBt != null ; pBt = pBt.pNext )//GLOBAL(BtShared,sqlite3SharedCacheList); pBt!=null; pBt=pBt.pNext)
{
Debug.Assert( pBt.nRef > 0 );
if ( ( zFullPathname.ToString() == sqlite3PagerFilename( pBt.pPager ) )
...
sqlite3_mutex_leave( mutexShared );
//sqlite3DbFree( db, zFullPathname );
}
#if SQLITE_DEBUG
else
{
...
p.sharable = true;
      ...
      {
        /*
        ** The following Debug.Asserts make sure that structures used by the btree are
        ...
        */
        ...
        if ( rc == SQLITE_OK )
        {
          rc = sqlite3PagerReadFileheader( pBt.pPager, zDbHeader.Length, zDbHeader );//zDbHeader.Length, zDbHeader);
        }
        if ( rc != SQLITE_OK )
        {
          goto btree_open_out;
...
pBt.nRef = 1;
mutexShared = sqlite3MutexAlloc( SQLITE_MUTEX_STATIC_MASTER );
if ( SQLITE_THREADSAFE != 0 && sqlite3Config.bCoreMutex )
{
...
if ( p.pBt.GetHashCode() < pSib.pBt.GetHashCode() )
{
p.pNext = pSib;
p.pPrev = null;
pSib.pPrev = p;
...
{
while ( pSib.pNext != null && pSib.pNext.pBt.GetHashCode() < p.pBt.GetHashCode() )
{
pSib = pSib.pNext;
}
p.pNext = pSib.pNext;
...
btree_open_out:
      if ( rc != SQLITE_OK )
      {
        ...
        pBt = null; // //sqlite3DbFree(db,ref pBt);
...
if ( pBt.nRef <= 0 )
{
if ( sqlite3SharedCacheList == pBt )//if( GLOBAL(BtShared,sqlite3SharedCacheList==pBt )!=0)
{
sqlite3SharedCacheList = pBt.pNext;//GLOBAL(BtShared,sqlite3SharedCacheList = pBt.pNext);
}
else
{
pList = sqlite3SharedCacheList; //GLOBAL( BtShared, sqlite3SharedCacheList );
...
{
sqlite3_mutex_free( ref pBt.mutex );
}
removed = true;
}
...
if ( p.pNext != null ) p.pNext.pPrev = p.pPrev;
    ...
    */
    ...
    static int sqlite3BtreeSetSafetyLevel( Btree p, int level, bool fullSync )
    {
      BtShared pBt = p.pBt;
      Debug.Assert( sqlite3_mutex_held( p.db.mutex ) );
      sqlite3BtreeEnter( p );
...
*/
    ...
    */
    static i64 sqlite3BtreeMaxPageCount( Btree p, int mxPage )
    {
      i64 n;
      sqlite3BtreeEnter( p );
...
return BTREE_AUTOVACUUM_NONE;
    ...
    */
    static int lockBtree( BtShared pBt )
    {
      ...
      Debug.Assert( sqlite3_mutex_held( pBt.mutex ) );
      Debug.Assert( pBt.pPage1 == null );
      rc = sqlite3BtreeGetPage( pBt, 1, ref pPage1, 0 );
        ...
        */
        ...
        if ( pageSize != pBt.pageSize )
        {
          ...
          */
          ...
          pageSize - usableSize );
          if ( rc != 0 ) goto page1_init_failed;
          return SQLITE_OK;
        }
        if ( usableSize < 480 )
        {
...
page1_init_failed:
    ...
    */
    static int lockBtreeWithRetry( Btree pRef )
    {
      ...
      Debug.Assert( sqlite3BtreeHoldsMutex( pRef ) );
      if ( pRef.inTrans == TRANS_NONE )
      {
        byte inTransaction = pRef.pBt.inTransaction;
#if DEBUG
        btreeIntegrity( pRef );
#endif
    ...
    */
    ...
    {
      Debug.Assert( sqlite3_mutex_held( pBt.mutex ) );
      if ( pBt.inTransaction == TRANS_NONE && pBt.pCursor == null && pBt.pPage1 != null )
      {
...
*/
      if ( p.inTrans == TRANS_WRITE || ( p.inTrans == TRANS_READ && wrflag == 0 ) )
      {
        goto trans_begun;
      }
...
*/
if ( ( wrflag != 0 && pBt.inTransaction == TRANS_WRITE ) || pBt.isPending != 0 )
{
...
if ( wrflag != 0 )
{
Debug.Assert( null == pBt.pWriter );
pBt.pWriter = p;
pBt.isExclusive = (byte)( wrflag > 1 ? 1 : 0 );
}
#endif
      }
...
*/
    static int setChildPtrmaps( MemPage pPage )
    {
      ...
      for ( i = 0 ; i < nCell ; i++ )
      {
        ...
        if ( 0 == pPage.leaf )
        {
          u32 childPgno = (u32)sqlite3Get4byte( pPage.aData, pCell );
      ...
      if ( 0 == pPage.leaf )
      {
        u32 childPgno = (u32)sqlite3Get4byte( pPage.aData, pPage.hdrOffset + 8 );
        rc = ptrmapPut( pBt, childPgno, PTRMAP_BTREE, pgno );
      }
...
set_child_ptrmaps_out:
    ...
    */
    static int modifyPagePointer( MemPage pPage, u32 iFrom, u32 iTo, int eType )
    {
      Debug.Assert( sqlite3_mutex_held( pPage.pBt.mutex ) );
      Debug.Assert( sqlite3PagerIswriteable( pPage.pDbPage ) );
        ...
        for ( i = 0 ; i < nCell ; i++ )
        {
          ...
          if ( eType == PTRMAP_OVERFLOW1 )
          {
            ...
            if ( info.iOverflow != 0 )
            {
              ...
              {
                sqlite3Put4byte( pPage.aData, pCell + info.iOverflow, iTo );
                break;
              }
            }
          }
          else
          {
            ...
            {
              sqlite3Put4byte( pPage.aData, pCell, iTo );
              break;
            }
          }
        }
...
return SQLITE_CORRUPT_BKPT;
    ...
    */
    ...
    int eType,                /* Pointer map 'type' entry for pDbPage */
    ...
    )
    {
      ...
      */
      ...
      {
        u32 nextOvfl = (u32)sqlite3Get4byte( pDbPage.aData );
        if ( nextOvfl != 0 )
        {
      ...
      */
      ...
      {
        rc = sqlite3BtreeGetPage( pBt, iPtrPage, ref pPtrPage, 0 );
        if ( rc != SQLITE_OK )
        {
          return rc;
        }
    ...
    */
    static int incrVacuumStep( BtShared pBt, Pgno nFin, Pgno iLastPg )
    {
      ...
      if ( !PTRMAP_ISPAGE( pBt, iLastPg ) && iLastPg != PENDING_BYTE_PAGE( pBt ) )
      {
        ...
        nFreeList = (u32)sqlite3Get4byte( pBt.pPage1.aData, 36 );
        if ( nFreeList == 0 )
        {
          return SQLITE_DONE;
        }
...
return SQLITE_CORRUPT_BKPT;
        ...
        if ( eType == PTRMAP_FREEPAGE )
        {
          if ( nFin == 0 )
          {
            ...
            */
            ...
            rc = allocateBtreePage( pBt, ref pFreePg, ref iFreePg, iLastPg, true );
            if ( rc != SQLITE_OK )
            {
              return rc;
            }
        ...
        {
          u32 iFreePg = 0;             /* Index of free page to move pLastPg to */
          MemPage pLastPg = null;

          rc = sqlite3BtreeGetPage( pBt, iLastPg, ref  pLastPg, 0 );
          if ( rc != SQLITE_OK )
          {
            return rc;
          }
          ...
          */
          ...
          {
            MemPage pFreePg = null;
            rc = allocateBtreePage( pBt, ref pFreePg, ref iFreePg, 0, false );
            if ( rc != SQLITE_OK )
            {
              releasePage( pLastPg );
              return rc;
          ...
          if ( rc == SQLITE_OK )
          {
            rc = relocatePage( pBt, pLastPg, eType, (u32)iPtrPage, iFreePg, ( nFin != 0 ) ? 1 : 0 );
          }
          releasePage( pLastPg );
          if ( rc != SQLITE_OK )
          {
      ...
      if ( nFin == 0 )
      {
        ...
        while ( iLastPg == PENDING_BYTE_PAGE( pBt ) || PTRMAP_ISPAGE( pBt, iLastPg ) )
        {
          ...
          {
            MemPage pPg = null;
            int rc = sqlite3BtreeGetPage( pBt, iLastPg, ref pPg, 0 );
            if ( rc != SQLITE_OK )
            {
              return rc;
            }
    ...
    */
    static int autoVacuumCommit( BtShared pBt )
    {
      ...
      if ( !pBt.incrVacuum )
      {
        ...
        u32 nOrig = (u32)pagerPagecount( pBt );
...
return SQLITE_CORRUPT_BKPT;
        ...
        nPtrmap = (u32)( (int)( nFree - nOrig + PTRMAP_PAGENO( pBt, nOrig ) + pgsz / 5 ) / ( pgsz / 5 ) );
...
#else
static int setChildPtrmaps( MemPage pPage ) { return SQLITE_OK; }//# define setChildPtrmaps(x) SQLITE_OK
...
#else
static int countWriteCursors( BtShared pBt ) { return 0; }
#endif
...
sqlite3BtreeTripAllCursors( p, rc );
      ...
      if ( p.inTrans == TRANS_WRITE )
      {
        ...
        ** sure pPage1.aData is set correctly. */
        if ( sqlite3BtreeGetPage( pBt, 1, ref pPage1, 0 ) == SQLITE_OK )
        {
          releasePage( pPage1 );
        }
    ...
    */
    ...
    )
    {
      ...
      if ( wrFlag != 0 )
      {
        ...
        rc = checkForReadConflicts( p, (Pgno)iTable, null, 0 );
        if ( rc != SQLITE_OK )
        {
          Debug.Assert( rc == SQLITE_LOCKED_SHAREDCACHE );
          return rc;
        }
      }
      ...
      rc = getAndInitPage( pBt, (u32)pCur.pgnoRoot, ref pCur.apPage[0] );
      if ( rc != SQLITE_OK )
      {
        goto create_cursor_exception;
      }
...
*/
    static void sqlite3BtreeGetTempCursor( BtCursor pCur, ref BtCursor pTempCur )
    {
      int i;
      Debug.Assert( cursorHoldsMutex( pCur ) );
      pTempCur = pCur.Copy();//memcpy(pTempCur, pCur, sizeof(BtCursor));
      pTempCur.pNext = null;
      pTempCur.pPrev = null;
      ...
      Debug.Assert( pTempCur.pKey == null );
    }
#endif //* SQLITE_TEST */
...
*/
    ...
    static void assertCellInfo( BtCursor pCur )
    {
      ...
      Debug.Assert( info.Equals( pCur.info ) );//memcmp( info, pCur.info, sizeof( ))== 0 );
    }
#else
//  #define Debug.AssertCellInfo(x)
#endif
#if  _MSC_VER
...
*/
    ...
    */
    ...
    BtShared pBt,
    u32 ovfl,                 /* Overflow page */
    ref MemPage ppPage,        /* OUT: MemPage handle (may be NULL) */
    ref u32 pPgnoNext          /* OUT: Next overflow page number */
    )
    {
      u32 next = 0;
      MemPage pPage = null;
...
*/
      ...
      if ( rc == SQLITE_OK )
      {
        rc = sqlite3BtreeGetPage( pBt, ovfl, ref pPage, 0 );
    ...
    */
    ...
    bool skipKey,        /* offset begins at data if this is true */
    ...
    )
    {
      ...
      u32 nKey;
      int iIdx = 0;
      MemPage pPage = pCur.apPage[pCur.iPage];  /* Btree page of current cursor entry */
      ...
      nKey = (u32)( pPage.intKey != 0 ? 0 : pCur.info.nKey );
...
return SQLITE_CORRUPT_BKPT;
      ...
      if ( offset < pCur.info.nLocal )
      {
        ...
        {
          a = (u32)( pCur.info.nLocal - offset );
        }
        rc = copyPayload( aPayload, (u32)( offset + pCur.info.iCell + pCur.info.nHeader ), pBuf, pBufOffset, a, eOp, pPage.pDbPage );
        offset = 0;
        pBufOffset += a;
        amt -= a;
      }
...
pCur.aOverflow = new u32[nOvfl];//(Pgno*)sqlite3MallocZero( Pgno.Length * nOvfl );
if ( nOvfl != 0 && pCur.aOverflow == null )
{
rc = SQLITE_NOMEM;
}
}
...
** directly to it.
*/
if ( pCur.aOverflow != null && pCur.aOverflow[offset] != 0 )/// ovflSize] )
{
iIdx = (int)( offset / ovflSize );
nextPage = (u32)pCur.aOverflow[iIdx];
...
{
Debug.Assert( pCur.aOverflow[iIdx] == 0 || pCur.aOverflow[iIdx] == nextPage );
...
nextPage = pCur.aOverflow[iIdx + 1];
          ...
          else
          {
            ...
            ** range of data that is being read (eOp==0) or written (eOp!=null).
            */
            DbPage pDbPage = new DbPage();
            u32 a = amt;
            ...
            if ( rc == SQLITE_OK )
            {
              ...
              rc = copyPayload( aPayload, offset + 4, pBuf, pBufOffset, a, eOp, pDbPage );
              sqlite3PagerUnref( pDbPage );
              offset = 0;
              amt -= a;
              pBufOffset += a;
...
return SQLITE_CORRUPT_BKPT;
    ...
    */
    static int sqlite3BtreeKey( BtCursor pCur, int offset, int amt, ref byte[] pBuf )
    {
      int rc;
...
return SQLITE_CORRUPT_BKPT;
        ...
        rc = accessPayload( pCur, (u32)offset, (u32)amt, pBuf, false, 0 );
      }
      return rc;
    }
...
return SQLITE_ABORT;
      ...
      if ( rc == SQLITE_OK )
      {
        ...
        Debug.Assert( pCur.iPage >= 0 && pCur.apPage[pCur.iPage] != null );
        Debug.Assert( pCur.aiIdx[pCur.iPage] < pCur.apPage[pCur.iPage].nCell );
        rc = accessPayload( pCur, offset, amt, pBuf, true, 0 );
      }
      return rc;
    }
    ...
    */
    static byte[] sqlite3BtreeKeyFetch( BtCursor pCur, ref int pAmt )
    {
      ...
      {
        return fetchPayload( pCur, ref pAmt, false );
      }
      return null;
    }
    static byte[] sqlite3BtreeDataFetch( BtCursor pCur, ref int pAmt )
    {
      ...
      if ( pCur.eState == CURSOR_VALID )
      {
        return fetchPayload( pCur, ref pAmt, true );
      }
      return null;
    }
...
** the page.
*/
    static void assertParentIndex( MemPage pParent, int iIdx, u32 iChild )
    {
...
*/
    ...
    */
    static int moveToRoot( BtCursor pCur )
    {
      ...
      {
        if (
        SQLITE_OK != ( rc = getAndInitPage( pBt, pCur.pgnoRoot, ref pCur.apPage[0] ) )
        )
        {
          pCur.eState = CURSOR_INVALID;
          return rc;
...
** to the last entry in the b-tree. */
      ...
      if ( rc == SQLITE_OK )
      {
        ...
        else
        {
          ...
          pCur.atLast = (byte)( rc == SQLITE_OK ? 1 : 0 );
        }
      }
      return rc;
    }
    ...
    )
    {
      ...
      ** to move to, then just return without doing any work */
      ...
      for ( ; ; )
      {
        ...
        lwr = 0;
        upr = pPage.nCell - 1;
        if ( ( 0 == pPage.intKey && pIdxKey == null ) || upr < 0 )
        {
#if SQLITE_DEBUG
...
rc = SQLITE_CORRUPT_BKPT;
        ...
        for ( ; ; )
        {
          ...
          if ( pPage.intKey != 0 )
          {
            ...
            {
              int dummy = 0;
              pCell += (u8)getVarint32( pPage.aData, pCell, ref dummy );
            }
          ...
          else
          {
            ...
            */
            ...
            else if ( 0 == ( pPage.aData[pCell + 1] & 0x80 )
            ...
            else
            {
              ...
              ** buffer before VdbeRecordCompare() can be called. */
              byte[] pCellKey;
              byte[] pCellBody = new byte[pPage.aData.Length - pCell + pPage.childPtrSize];
              ...
              pCellKey = new byte[nCell]; //pCellKey = sqlite3Malloc( nCell );
              if ( pCellKey == null )
              {
                rc = SQLITE_NOMEM;
                goto moveto_finish;
              }
              rc = accessPayload( pCur, 0, (uint)nCell, pCellKey, false, 0 );
...
moveto_finish:
    ...
    */
    ...
    bool bias,          /* Bias search to the high end */
    ref int pRes        /* Write search results here */
    )
    {
      int rc;                    /* Status code */
      ...
      if ( pKey != null )
      {
        Debug.Assert( nKey == (Pgno)(int)nKey );
      ...
      rc = sqlite3BtreeMovetoUnpacked( pCur, pIdxKey, nKey, bias ? 1 : 0, ref pRes );
      if ( pKey != null )
      {
    ...
    */
    static int sqlite3BtreeNext( BtCursor pCur, ref int pRes )
    {
      ...
      // Debug.Assert( pRes != 0 );  -- Not needed, since pRes is an integer
      if ( CURSOR_INVALID == pCur.eState )
      {
        pRes = 1;
        return SQLITE_OK;
...
return SQLITE_CORRUPT_BKPT;
      ...
      if ( n > 0 )
      {
        ...
        u32 iTrunk;
        int searchList = 0; /* If the free-list must be searched for 'nearby' */
...
*/
        ...
        */
        do
        {
          ...
          if ( pPrevTrunk != null )
          {
            iTrunk = sqlite3Get4byte( pPrevTrunk.aData );
          }
          else
          {
...
rc = SQLITE_CORRUPT_BKPT;
          ...
          {
            rc = sqlite3BtreeGetPage( pBt, iTrunk, ref  pTrunk, 0 );
          }
          if ( rc != 0 )
          {
            pTrunk = null;
          ...
          if ( k == 0 && searchList == 0 )
          {
            ...
            ** allocated page */
            ...
            pPgno = iTrunk;
            Buffer.BlockCopy( pTrunk.aData, 0, pPage1.aData, 32, 4 );//memcpy(pPage1.aData[32], pTrunk.aData[0], 4);
            ppPage = pTrunk;
            pTrunk = null;
            ...
            */
            ...
            else
            {
              ...
              */
              ...
              Pgno iNewTrunk = (Pgno)sqlite3Get4byte( pTrunk.aData, 8 );
              if ( iNewTrunk > mxPage )
              {
#if SQLITE_DEBUG
                rc = SQLITE_CORRUPT_BKPT();
...
rc = SQLITE_CORRUPT_BKPT;
              ...
              rc = sqlite3BtreeGetPage( pBt, iNewTrunk, ref pNewTrunk, 0 );
              if ( rc != SQLITE_OK )
              {
                goto end_allocate_page;
              }
              ...
              Buffer.BlockCopy( pTrunk.aData, 12, pNewTrunk.aData, 8, (int)( k - 1 ) * 4 );//memcpy(pNewTrunk.aData[8], pTrunk.aData,12, (k-1)*4);
              releasePage( pNewTrunk );
              if ( null == pPrevTrunk )
              {
...
rc = SQLITE_CORRUPT_BKPT;
              ...
              if ( closest < k - 1 )
              {
                Buffer.BlockCopy( aData, (int)( 4 + k * 4 ), aData, 8 + (int)closest * 4, 4 );// memcpy(&aData[8 + closest * 4], aData[4 + k * 4], 4);
              }
              sqlite3Put4byte( aData, (u32)4, (u32)( k - 1 ) );
              Debug.Assert( sqlite3PagerIswriteable( pPage1.pDbPage ) );
              noContent = !btreeGetHasContent( pBt, pPgno ) ? (u8)1 : (u8)0;
              rc = sqlite3BtreeGetPage( pBt, pPgno, ref ppPage, noContent );
              if ( rc == SQLITE_OK )
              {
      ...
      else
      {
        ...
        ** end of the file */
        u32 nPage = (u32)pagerPagecount( pBt );
        pPgno = nPage + 1;
        ...
        if ( pBt.autoVacuum && PTRMAP_ISPAGE( pBt, pPgno ) )
        {
          ...
          */
          ...
          rc = sqlite3BtreeGetPage( pBt, pPgno, ref pPg, 0 );
          if ( rc == SQLITE_OK )
          {
            rc = sqlite3PagerWrite( pPg.pDbPage );
            releasePage( pPg );
        ...
        Debug.Assert( pPgno != ( PENDING_BYTE / pBt.pageSize ) + 1 );//PENDING_BYTE_PAGE(pBt) );
        rc = sqlite3BtreeGetPage( pBt, pPgno, ref ppPage, 0 );
        if ( rc != 0 ) return rc;
        rc = sqlite3PagerWrite( ppPage.pDbPage );
        if ( rc != SQLITE_OK )
        {
...
return SQLITE_CORRUPT_BKPT;
    ...
    */
    static int freePage2( BtShared pBt, MemPage pMemPage, u32 iPage )
    {
      MemPage pTrunk = null;              /* Free-list trunk page */
      u32 iTrunk = 0;                     /* Page number of free-list trunk page */
      ...
      sqlite3Put4byte( pPage1.aData, (u32)36, nFree + 1 );
...
if (false)
      ...
      */
      if ( nFree != 0 )
      {
        ...
        if ( nLeaf < 0 || nLeaf > Int32.MaxValue )
        {
#if SQLITE_DEBUG
          rc = SQLITE_CORRUPT_BKPT();
#else
rc = SQLITE_CORRUPT_BKPT;
      ...
      */
      if ( ( ( null == pPage ) && ( 0 != ( rc = sqlite3BtreeGetPage( pBt, iPage, ref pPage, 0 ) ) ) )
...
return SQLITE_CORRUPT;
      ...
      while ( nPayload > 0 )
      {
        ...
        {
#if !SQLITE_OMIT_AUTOVACUUM
          u32 pgnoPtrmap = pgnoOvfl; /* Overflow page pointer-map entry page */
          if ( pBt.autoVacuum )
          {
            do
            {
...
#endif
          rc = allocateBtreePage( pBt, ref pOvfl, ref pgnoOvfl, pgnoOvfl, false );
#if !SQLITE_OMIT_AUTOVACUUM
...
*/
          ...
          {
            int eType = ( ( pgnoPtrmap != 0 ) ? PTRMAP_OVERFLOW2 : PTRMAP_OVERFLOW1 );
          ...
          ** is still writeable */
          ...
          pPayload = pOvfl.aData; pPayloadIndex = 4;
        ...
        ** is still writeable */
        ...
        else
        {
          ...
          Buffer.BlockCopy( pZeroBlob, 0, pPayload, pPayloadIndex, n );// memcpy(pPayload, pSrc, n);
        }
        nPayload -= n;
        pPayloadIndex += (int)n; // pPayload += n;
        pSrcIndex += (int)n; //    pSrc += n;
        nSrc -= n;
        spaceLeft -= (int)n;
        if ( nSrc == 0 )
        {
    ...
    */
    static int dropCell( MemPage pPage, int idx, int sz )
    {
      ...
      Debug.Assert( sqlite3_mutex_held( pPage.pBt.mutex ) );
      data = pPage.aData;
      ptr = pPage.cellOffset + 2 * idx; // &data[pPage.cellOffset + 2 * idx];
      pc = get2byte( data, ptr );
      if ( ( pc < pPage.hdrOffset + 6 + ( ( pPage.leaf != 0 ) ? 0 : 4 ) )
...
return SQLITE_CORRUPT_BKPT;
      ...
      pPage.nCell--;
      data[pPage.hdrOffset + 3] = (byte)( pPage.nCell >> 8 ); data[pPage.hdrOffset + 4] = (byte)( pPage.nCell );// put2byte(data, pPage.hdrOffset + 3, pPage.nCell);
      pPage.nFree += 2;
      return SQLITE_OK;
    }
    ...
    )
    {
      ...
      else
      {
        ...
        if ( end > top - sz )
        {
          ...
          top = get2byte( data, hdr + 5 );
          Debug.Assert( end + sz <= top );
        }
        idx = allocateSpace( pPage, sz );
        Debug.Assert( idx > 0 );
        Debug.Assert( end <= get2byte( data, hdr + 5 ) );
...
return SQLITE_CORRUPT_BKPT;
    ...
    */
    ...
    int[] offset      /* Offset into the cell body, for c#  */
    )
    {
      int i;            /* Loop counter */
      int pCellptr;      /* Address of next cell pointer */
      ...
      for ( i = nCell - 1 ; i >= 0 ; i-- )
      {
        ...
        cellbody -= aSize[i];
        put2byte( data, pCellptr, cellbody );
        Buffer.BlockCopy( apCell, offset[i], data, cellbody, aSize[i] );//          memcpy(&data[cellbody], apCell[i], aSize[i]);
        Debug.Assert( cellbody == pPage.pBt.usableSize );
      }
      put2byte( data, hdr + 3, nCell );
      put2byte( data, hdr + 5, cellbody );
      pPage.nFree -= (u16)( nCell * 2 + nUsable - cellbody );
    ...
    */
    const int NN = 1;                 /* Number of neighbors on either side of pPage */
    const int NB = ( NN * 2 + 1 );    /* Total pages involved in the balance */
...
*/
      rc = allocateBtreePage( pBt, ref pNew, ref pgnoNew, 0, false );
      if ( rc == SQLITE_OK )
      {
...
if (false)
#endif
        {
          ...
          if ( szCell[0] > pNew.minLocal && rc == SQLITE_OK )
          {
        ...
        */
        ...
        insertCell( pParent, pParent.nCell, pSpace, pOut, null, pPage.pgno );
...
*/
    static int copyNodeContent( MemPage pFrom, MemPage pTo )
    {
      ...
      Buffer.BlockCopy( aFrom, iData, aTo, iData, pBt.usableSize - iData );//memcpy( &aTo[iData], &aFrom[iData], pBt.usableSize - iData );
      Buffer.BlockCopy( aFrom, iFromHdr, aTo, iToHdr, pFrom.cellOffset + 2 * pFrom.nCell );//memcpy( &aTo[iToHdr], &aFrom[iFromHdr], pFrom.cellOffset + 2 * pFrom.nCell );
      ...
      ** data copied from pFrom is known to be valid.  */
      ...
      rc = sqlite3BtreeInitPage( pTo );//TESTONLY(rc = ) sqlite3BtreeInitPage(pTo);
#else
sqlite3BtreeInitPage( pTo );
#endif
...
if (false)
    ...
    */
    ...
    )
    {
      ...
      int[] apDiv = new int[NB - 1];        /* Divider cell location in pParent */
...
#if FALSE
TRACE( "BALANCE: begin page %d child of %d\n", sqlite3Config.pPage.pgno, pParent.pgno );
#endif
...
** is called (indirectly) from sqlite3BtreeDelete(). */
      ...
      ** been removed.  */
      ...
      while ( true )
      {
        ...
        if ( pParent.nOverflow != 0 && i + nxDiv == pParent.aOvfl[0].idx )
        {
...
apDiv[i] = aOvflSpace[apDiv[i]-pParent.aData];
      ...
      */
      ...
      aSpace1 = new byte[pBt.pageSize * ( nOld - 1 )];
      ...
      */
      ...
      for ( i = 0 ; i < nOld ; i++ )
      {
        ...
        ** process of being overwritten.  */
        //(MemPage*)&aSpace1[pBt->pageSize + k*i];
        ...
        if ( i < nOld - 1 && 0 == leafData )
        {
          ...
          Buffer.BlockCopy( pTemp, leafCorrection, apCell[nCell], 0, sz );//  = pTemp + leafCorrection;
          ...
          if ( pOld.leaf == 0 )
          {
            ...
            ** pointer of the divider cell */
            Buffer.BlockCopy( pOld.aData, 8, apCell[nCell], 0, 4 );//memcpy(apCell[nCell], &pOld->aData[8], 4);
          }
          else
          {
...
rc = SQLITE_CORRUPT_BKPT;
      ...
      */
      ...
      /* Either we found one or more cells (cntnew[0])>0) or  or pPage is
...
rc = SQLITE_CORRUPT_BKPT;
      ...
      for ( i = 0 ; i < k ; i++ )
      {
        ...
        {
          Debug.Assert( i > 0 );
          rc = allocateBtreePage( pBt, ref pNew, ref pgno, pgno, false );
          if ( rc != 0 ) goto balance_cleanup;
          apNew[i] = pNew;
          nNew++;
      ...
      for ( i = 0 ; i < nNew ; i++ )
      {
        ...
        */
        ...
        if ( j < nCell )
        {
          ...
          pTemp = new byte[sz];//pTemp = aOvflSpace[iOvflSpace];
          if ( pNew.leaf == 0 )
          {
          ...
          else if ( leafData != 0 )
          {
            ...
            */
            ...
            pCell = new byte[sz + 4];//pTemp + Header size
...
#endif
        {
          ...
          ** actually moved between pages.  */
          ...
          for ( i = 0 ; i < nCell && rc == SQLITE_OK ; i++ )
          {
            ...
            ** with any child or overflow pages need to be updated.  */
            if ( isDivider != 0 || pOld.pgno != pNew.pgno )
            {
              if ( 0 == leafCorrection )
              {
                rc = ptrmapPut( pBt, sqlite3Get4byte( apCell[i] ), PTRMAP_BTREE, pNew.pgno );
              }
          ...
          if ( 0 == leafCorrection )
          {
            ...
            {
              rc = ptrmapPut(
              pBt, sqlite3Get4byte( apNew[i].aData, 8 ), PTRMAP_BTREE, apNew[i].pgno );
            }
          }
...
balance_cleanup:
      ...
      for ( i = 0 ; i < nOld ; i++ )
      {
        if ( apOld[i] != null ) releasePage( apOld[i] );
      }
      for ( i = 0 ; i < nNew ; i++ )
      {
        releasePage( apNew[i] );
    ...
    */
    static int balance_deeper( MemPage pRoot, ref MemPage ppChild )
    {
      ...
      */
      ...
      || SQLITE_OK != ( rc = allocateBtreePage( pBt, ref pChild, ref pgnoChild, pRoot.pgno, false ) )
        ...
        else
        {
          ...
          if ( rc == SQLITE_OK )
          {
...
#endif
            {
              ...
              ** pSpace buffer pDebug.Assed to the latter call to balance_nonroot().
              */
              u8[] pSpace = new u8[pCur.pBt.pageSize];// sqlite3PageMalloc( pCur.pBt.pageSize );
...
|| p.isIncrblobHandle
    ...
    */
    ...
    bool appendBias,               /* True if this is likely an append */
    ...
    )
    {
      ...
      */
      if (
      SQLITE_OK != ( rc = saveAllCursors( pBt, pCur.pgnoRoot, pCur ) ) || ( 0 == loc &&
      SQLITE_OK != ( rc = sqlite3BtreeMoveto( pCur, pKey, nKey, appendBias, ref loc ) )
      ) )
      {
        return rc;
      }
      ...
      Debug.Assert( pPage.leaf != 0 || pPage.intKey == 0 );
      ...
      rc = insertCell( pPage, idx, newCell, szNew, null, 0 );
    ...
    */
    static int sqlite3BtreeDelete( BtCursor pCur )
    {
      ...
      ** balancing the tree following the delete operation easier.  */
      ...
      {
        int notUsed = 0;
        if ( SQLITE_OK != ( rc = sqlite3BtreePrevious( pCur, ref notUsed ) ) )
        {
          return rc;
        }
      }
      ...
      ** node to replace the deleted cell.  */
      if ( 0 == pPage.leaf )
      {
        ...
        if ( SQLITE_OK != ( rc = sqlite3PagerWrite( pLeaf.pDbPage ) ) ) return rc;
        ...
        if ( SQLITE_OK != ( rc = insertCell( pPage, iCellIdx, pNext_4, nCell + 4, null, n ) ) ) return rc;
        if ( SQLITE_OK != ( rc = dropCell( pLeaf, pLeaf.nCell - 1, nCell ) ) ) { return rc; }
      }
...
#if  SQLITE_OMIT_AUTOVACUUM
rc = allocateBtreePage( pBt, ref pRoot, ref pgnoRoot, 1, false );
if ( rc != 0 )
{
return rc;
}
      ...
      if ( pBt.autoVacuum )
      {
        u32 pgnoMove = 0;      /* Move a page here to make room for the root-page */
        ...
        */
        rc = sqlite3BtreeGetMeta( p, BTREE_LARGEST_ROOT_PAGE, ref pgnoRoot );
        if ( rc != SQLITE_OK )
        {
          return rc;
        }
        ...
        ** PENDING_BYTE page.
        */
        while ( pgnoRoot == PTRMAP_PAGENO( pBt, (u32)pgnoRoot ) ||
        ...
        */
        rc = allocateBtreePage( pBt, ref pPageMove, ref pgnoMove, (u32)pgnoRoot, true );
        if ( rc != SQLITE_OK )
        {
          return rc;
        }
        ...
        if ( pgnoMove != pgnoRoot )
        {
          ...
          */
          ...
          rc = sqlite3BtreeGetPage( pBt, (u32)pgnoRoot, ref pRoot, 0 );
          if ( rc != SQLITE_OK )
          {
            return rc;
          }
          rc = ptrmapGet( pBt, (u32)pgnoRoot, ref eType, ref iPtrPage );
...
rc = SQLITE_CORRUPT_BKPT;
        ...
        rc = ptrmapPut( pBt, (u32)pgnoRoot, PTRMAP_ROOTPAGE, 0 );
        if ( rc != 0 )
        {
          releasePage( pRoot );
          return rc;
        }
        rc = sqlite3BtreeUpdateMeta( p, 4, (u32)pgnoRoot );
        if ( rc != 0 )
        {
          releasePage( pRoot );
          return rc;
      ...
      {
        Pgno iDummy = 0;
        rc = allocateBtreePage( pBt, ref pRoot, ref iDummy, 1, false );
        pgnoRoot = iDummy;
        if ( rc != 0 ) return rc;
      }
#endif
    ...
    */
    ...
    u32 pgno,             /* Page number to clear */
    bool freePageFlag,    /* Deallocate page if true */
    ref int pnChange
    )
    {
      MemPage pPage = null;
...
return SQLITE_CORRUPT_BKPT;
      ...
      for ( i = 0 ; i < pPage.nCell ; i++ )
      {
        ...
        if ( pPage.leaf == 0 )
        {
          rc = clearDatabasePage( pBt, sqlite3Get4byte( pCell, iCell ), true, ref pnChange );
          if ( rc != 0 ) goto cleardatabasepage_out;
        }
      ...
      if ( pPage.leaf == 0 )
      {
        rc = clearDatabasePage( pBt, sqlite3Get4byte( pPage.aData, 8 ), true, ref  pnChange );
        if ( rc != 0 ) goto cleardatabasepage_out;
      }
      else // Under C#, parameter always exists if ( pnChange != 0 )
      {
...
cleardatabasepage_out:
    ...
    */
    static int sqlite3BtreeClearTable( Btree p, int iTable, ref  int pnChange )
    {
      ...
      sqlite3BtreeEnter( p );
      Debug.Assert( p.inTrans == TRANS_WRITE );
      if ( ( rc = checkForReadConflicts( p, (Pgno)iTable, null, 1 ) ) != SQLITE_OK )
      {
        /* nothing to do */
      }
      else if ( SQLITE_OK != ( rc = saveAllCursors( pBt, (Pgno)iTable, null ) ) )
      {
        /* nothing to do */
      }
      else
      {
        rc = clearDatabasePage( pBt, (u32)iTable, false, ref  pnChange );
      }
      sqlite3BtreeLeave( p );
      return rc;
    }
    ...
    */
    static int btreeDropTable( Btree p, Pgno iTable, ref int piMoved )
    {
      ...
      */
      ...
      rc = sqlite3BtreeGetPage( pBt, iTable, ref pPage, 0 );
      if ( rc != 0 ) return rc;
      int iDummy = 0;
      rc = sqlite3BtreeClearTable( p, (int)iTable, ref iDummy );
      if ( rc != 0 )
      {
        releasePage( pPage );
        return rc;
...
releasePage( pPage );
        ...
        {
          Pgno maxRootPgno = 0;
          rc = sqlite3BtreeGetMeta( p, BTREE_LARGEST_ROOT_PAGE, ref maxRootPgno );
          if ( rc != SQLITE_OK )
          {
            releasePage( pPage );
            return rc;
          ...
          else
          {
            ...
            */
            MemPage pMove = null;
            releasePage( pPage );
            rc = sqlite3BtreeGetPage( pBt, (u32)maxRootPgno, ref pMove, 0 );
            if ( rc != SQLITE_OK )
            {
              return rc;
            }
          ...
          */
          ...
          if ( maxRootPgno == PTRMAP_PAGENO( pBt, (u32)maxRootPgno ) )
          {
            maxRootPgno--;
          }
          Debug.Assert( maxRootPgno != PENDING_BYTE_PAGE( pBt ) );

          rc = sqlite3BtreeUpdateMeta( p, 4, (u32)maxRootPgno );
        }
        else
        {
          rc = freePage( pPage );
    ...
    static int sqlite3BtreeDropTable( Btree p, u32 iTable, ref int piMoved )
    {
      int rc;
      sqlite3BtreeEnter( p );
      rc = btreeDropTable( p, iTable, ref piMoved );
      sqlite3BtreeLeave( p );
      return rc;
    }
    ...
    */
    static int sqlite3BtreeGetMeta( Btree p, int idx, ref u32 pMeta )
    {
      DbPage pDbPage = null;
      int rc;
      byte[] pP1;
      ...
      */
      ...
      if ( pBt.pPage1 != null )
      {
        ...
        */
        pP1 = pBt.pPage1.aData;//(unsigned char *)
      }
      else
      {
        ...
        */
        rc = sqlite3PagerGet( pBt.pPager, 1, ref pDbPage );
        if ( rc != 0 )
        {
          sqlite3BtreeLeave( p );
          return rc;
        }
        pP1 = sqlite3PagerGetData( pDbPage );
      }
      pMeta = sqlite3Get4byte( pP1, 36 + idx * 4 );
      ...
      */
#if  SQLITE_OMIT_AUTOVACUUM
if ( idx == BTREE_LARGEST_ROOT_PAGE && pMeta > 0 ) pBt.readOnly = true;
#endif
...
#endif
      Debug.Assert( pPage != null );
      Debug.Assert( pPage.pBt == pCur.pBt );
      return (int)pPage.aData[pPage.hdrOffset];
    }
      ...
      */
      while ( rc == SQLITE_OK )
      {
        ...
        */
        ...
        if ( iIdx == pPage.nCell )
        {
          rc = moveToChild( pCur, sqlite3Get4byte( pPage.aData, pPage.hdrOffset + 8 ) );//sqlite3Get4byte
        }
        else
        {
          rc = moveToChild( pCur, sqlite3Get4byte( pPage.aData, findCell( pPage, iIdx ) ) );//sqlite3Get4byte
        }
      }
...
*/
    ...
    )
    {
      ...
      va_end( ap );
      //if ( pCheck.errMsg.mallocFailed != 0 )
      //{
...
*/
    ...
    u32 iChild,            /* Child page number */
    int eType,             /* Expected pointer map type */
    u32 iParent,           /* Expected pointer map parent page number */
...
*/
    ...
    bool isFreeList,       /* True for a freelist.  False for overflow page list */
    u32 iPage,             /* Page number for first page in the list */
    ...
    )
    {
      ...
      while ( N-- > 0 && pCheck.mxErr != 0 )
      {
        ...
        if ( checkRef( pCheck, iPage, zContext ) != 0 ) break;
        if ( sqlite3PagerGet( pCheck.pPager, iPage, ref pOvflPage ) != 0 )
        {
        ...
        if ( isFreeList )
        {
          u32 n = sqlite3Get4byte( pOvflData, 4 );
#if !SQLITE_OMIT_AUTOVACUUM
          ...
          else
          {
            for ( i = 0 ; i < n ; i++ )
            {
              u32 iFreePage = sqlite3Get4byte( pOvflData, 8 + i * 4 );
#if !SQLITE_OMIT_AUTOVACUUM
              if ( pCheck.pBt.autoVacuum )
              {
                checkPtrmap( pCheck, (u32)iFreePage, PTRMAP_FREEPAGE, 0, zContext );
              }
#endif
...
*/
    ...
    u32 iPage,            /* Page number of the page to check */
    string zParentContext /* Parent context */
    )
    {
      MemPage pPage = new MemPage();
      int i, rc, depth, d2, cnt;
      u32 pgno;
      int hdr, cellStart;
      int nCell;
      byte[] data;
      ...
      sqlite3_snprintf( 100, ref zContext, "Page %d: ", iPage );
      ...
      */
      ...
      usableSize = pBt.usableSize;
      if ( iPage == 0 ) return 0;
      if ( checkRef( pCheck, iPage, zParentContext ) != 0 ) return 0;
      if ( ( rc = sqlite3BtreeGetPage( pBt, iPage, ref pPage, 0 ) ) != 0 )
      {
        //if ( rc == SQLITE_NOMEM || rc == SQLITE_IOERR_NOMEM ) pCheck.mallocFailed = 1;
        checkAppendMsg( pCheck, zContext,
      ...
      */
      ...
      for ( i = 0 ; i < pPage.nCell && pCheck.mxErr != 0 ; i++ )
      {
        ...
        */
        ...
        pCell = pPage.aData;
        sqlite3BtreeParseCellPtr( pPage, iCell, info );//pCell, info);
        sz = info.nData;
        if ( pPage.intKey == 0 ) sz += (u32)info.nKey;
        Debug.Assert( sz == info.nPayload );
        if ( ( sz > info.nLocal ) //     && (pCell[info.iOverflow]<=pPage.aData[pBt.usableSize])
        )
        {
          u32 nPage = (u32)( ( sz - info.nLocal + usableSize - 5 ) / ( usableSize - 4 ) );
          u32 pgnoOvfl = sqlite3Get4byte( pCell, iCell, info.iOverflow );
#if !SQLITE_OMIT_AUTOVACUUM
          if ( pBt.autoVacuum )
          {
...
#endif
          checkList( pCheck, false, pgnoOvfl, nPage, zContext );
        }
        ...
        */
        if ( pPage.leaf == 0 )
        {
          pgno = sqlite3Get4byte( pCell, iCell );
#if !SQLITE_OMIT_AUTOVACUUM
          if ( pBt.autoVacuum )
          {
      ...
      */
      ...
      else
      {
        ...
        for ( i = 0 ; i < nCell ; i++ )
        {
          ...
          if ( pc <= usableSize )
          {
            size = cellSizePtr( pPage, pc );// data[pc]);
          }
...
check_page_abort:
      if ( hit != null ) sqlite3PageFree( ref   hit );
...
return sqlite3DbStrDup(0, "cannot acquire a read lock on the database");
#endif
        return "Unable to acquire a read lock on the database";// sqlite3StrDup( "Unable to acquire a read lock on the database" );
      }
      sCheck.pBt = pBt;
      ...
      i = (u32)( ( PENDING_BYTE / pBt.pageSize ) + 1 );// PENDING_BYTE_PAGE( pBt );
      if ( i <= sCheck.nPage )
      {
        sCheck.anRef[i] = 1;
      }
      ...
      */
      checkList( sCheck, true, sqlite3Get4byte( pBt.pPage1.aData, 32 ),
...
#endif
        checkTreePage( sCheck, (u32)aRoot[i], "List of tree roots: " );
      }
...
*/
    ...
    ** blob of allocated memory. This function should not call //sqlite3DbFree(db,)
...
if ( p.sharable )
{
u8 lockType = (u8)( READ_LOCK + isWriteLock );
Debug.Assert( READ_LOCK + 1 == WRITE_LOCK );
Debug.Assert( isWriteLock == 0 || isWriteLock == 1 );
sqlite3BtreeEnter( p );
rc = querySharedCacheTableLock( p, (uint)iTab, lockType );
if ( rc == SQLITE_OK )
{
rc = setSharedCacheTableLock( p, (uint)iTab, lockType );
}
sqlite3BtreeLeave( p );
}
return rc;
...
*/
int sqlite3BtreePutData( BtCursor pCsr, u32 offset, u32 amt, byte[] z )
{
int rc;
...
if ( pCsr.eState == CURSOR_INVALID || 0 == pCsr.apPage[pCsr.iPage].intKey )
{
return SQLITE_ERROR;
}

return accessPayload( pCsr, offset, amt, z, false, 1 );
}
...
Debug.Assert( pCur.aOverflow == null );
pCur.isIncrblobHandle = true;
}
#endif
  }
}
